...
import { IBasicPoint, Point } from './lib/point';
...
@Component({
	selector: 'signature-draw-pad',
	template: '<canvas></canvas>',
	styleUrls: ['./signature-draw-pad.page.scss'],
})
export class SignatureDrawPad {
	...
	public initSignaturePad(options: IOptions) {
		...
		if (this.throttle) {
			this._strokeMoveUpdate = throttle(
				SignatureDrawPad.prototype._strokeUpdate,
				this.throttle,
			);
		} else {
			this._strokeMoveUpdate = SignatureDrawPad.prototype._strokeUpdate;
		}

		this.dotSize =
			options.dotSize ||
			function dotSize(this: SignatureDrawPad) {
	...
	private _fromData(
		...
		drawCurve: SignatureDrawPad['_drawCurve'],
		drawDot: SignatureDrawPad['_drawDot'],
	): void {
